#!/usr/bin/env python3
"""Diffraction from homogeneous and twisted-nematic polarization gratings."""
...
parser.add_argument(
    "--validation",
    action="store_true",
    help="run a bounded anisotropic CPU/CUDA oracle and retain every mode power",
)
...
dd = (
    np.asarray([0.7, 1.7, 2.7])
    if args.validation
    else np.arange(0.1, 3.5, 0.1)
)  # chiral layer thickness
...
polarization_input_flux = np.zeros((2, dd.size))
polarization_angles = np.zeros((2, dd.size, nmode))
polarization_coeff_magnitudes = np.zeros((2, dd.size, 2, nmode))
polarization_transmission = np.zeros((2, dd.size, nmode))
...
for k in range(len(dd)):
    ...
    polarization_input_flux[0, k] = input_flux
    polarization_angles[0, k] = angles
    polarization_coeff_magnitudes[0, k, 0] = np.abs(coeffs1)
    polarization_coeff_magnitudes[0, k, 1] = np.abs(coeffs2)
    polarization_transmission[0, k] = tran
    ...
    tran = (abs(coeffs1) ** 2 + abs(coeffs2) ** 2) / input_flux
    polarization_input_flux[1, k] = input_flux
    polarization_angles[1, k] = angles
    polarization_coeff_magnitudes[1, k, 0] = np.abs(coeffs1)
    polarization_coeff_magnitudes[1, k, 1] = np.abs(coeffs2)
    polarization_transmission[1, k] = tran
    for m in range(nmode):
...
polarization_thicknesses = np.asarray(dd)
polarization_phase = np.asarray(phase)
polarization_uniaxial_efficiency = np.column_stack([eff_m0, eff_m1])
polarization_uniaxial_analytic = np.column_stack(
    [eff_m0_analytic, eff_m1_analytic]
)
polarization_uniaxial_error = np.abs(
    polarization_uniaxial_efficiency - polarization_uniaxial_analytic
)
...
polarization_twisted_efficiency = np.column_stack([eff_m0, eff_m1])
polarization_branch_separation = np.asarray(
    [
        np.linalg.norm(
            polarization_transmission[0] - polarization_transmission[1]
        )
    ]
)

if args.validation:
    # Isolate the effect of director twist from the original example's
    # simultaneous thickness change.  The twisted branch above already uses
    # d=dd[k]; rerun ph=0 at exactly that d for one representative thickness.
    matched_index = len(dd) // 2
    (
        matched_input_flux,
        matched_angles,
        matched_coeffs1,
        matched_coeffs2,
    ) = pol_grating(dd[matched_index], math.radians(ph_uniaxial), gp, nmode)
    polarization_matched_control_input_flux = np.asarray([matched_input_flux])
    polarization_matched_control_angles = np.asarray(matched_angles)
    polarization_matched_control_coeff_magnitudes = np.stack(
        [np.abs(matched_coeffs1), np.abs(matched_coeffs2)]
    )
    polarization_matched_control_transmission = (
        np.abs(matched_coeffs1) ** 2 + np.abs(matched_coeffs2) ** 2
    ) / matched_input_flux
    polarization_twist_only_separation = np.asarray(
        [
            np.linalg.norm(
                polarization_matched_control_transmission
                - polarization_transmission[1, matched_index]
            )
        ]
    )
    retained = (
        polarization_input_flux,
        polarization_angles,
        polarization_coeff_magnitudes,
        polarization_transmission,
        polarization_uniaxial_efficiency,
        polarization_uniaxial_analytic,
        polarization_uniaxial_error,
        polarization_twisted_efficiency,
        polarization_branch_separation,
        polarization_matched_control_input_flux,
        polarization_matched_control_angles,
        polarization_matched_control_coeff_magnitudes,
        polarization_matched_control_transmission,
        polarization_twist_only_separation,
    )
    if not all(np.all(np.isfinite(values)) for values in retained):
        raise RuntimeError("polarization-grating validation produced nonfinite data")
    if np.min(polarization_input_flux) <= 1e-6:
        raise RuntimeError("polarization-grating reference flux is empty")
    if np.min(polarization_transmission) < 0:
        raise RuntimeError("polarization-grating modal power is negative")
    if np.max(polarization_uniaxial_error) > 0.18:
        raise RuntimeError(
            "uniaxial polarization-grating efficiencies disagree with theory: "
            f"{polarization_uniaxial_error.tolist()}"
        )
    if polarization_branch_separation[0] <= 0.02:
        raise RuntimeError("twisted and uniaxial gratings are not physically distinct")
    if polarization_twist_only_separation[0] <= 0.1:
        raise RuntimeError(
            "director twist has no effect at matched liquid-crystal thickness"
        )
