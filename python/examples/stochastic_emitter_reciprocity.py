...
courant = 0.5  # timestep divided by spatial resolution
...
def _forward_problem(n: int, is_textured: bool):
    """Constructs the forward simulation and its flux monitor."""
    sources = [
        mp.Source(
...
def forward(n: int, rt: int, is_textured: bool) -> [List, np.ndarray]:
    ...
    """
    sim, flux_mon = _forward_problem(n, is_textured)
...
def backward(
    rt: int, is_textured: bool, return_metadata: bool = False
) -> [List, np.ndarray]:
    ...
    Returns:
      The frequency and Poynting flux spectra. If `return_metadata` is true,
      also returns the x coordinates, cubature weights, and per-frequency raw
      |Ez|^2 samples used for the spatial integral.
    """
    sources = [
        mp.Source(
            mp.GaussianSource(fcen, fwidth=df),
    ...
    try:
        ...
        (
            ...
            z_metadata,
            metadata_weights,
        ) = sim.get_array_metadata(dft_cell=dft_mon)
        x_metadata = np.asarray(x_metadata, dtype=float).copy()
        y_metadata = np.asarray(y_metadata, dtype=float).copy()
        z_metadata = np.asarray(z_metadata, dtype=float).copy()
        metadata_weights = np.asarray(metadata_weights, dtype=float).reshape(-1).copy()

        raw_power = []
        for nf in range(nfreq):
            dft_ez = np.asarray(sim.get_dft_array(dft_mon, mp.Ez, nf))
            raw_power.append(np.abs(dft_ez.reshape(-1)) ** 2)
        raw_power = np.asarray(raw_power, dtype=float)
        if raw_power.shape[1:] != metadata_weights.shape:
            raise RuntimeError(
                "backward DFT samples and cubature weights have different shapes"
            )
        abs_flux = np.sum(raw_power * metadata_weights[np.newaxis, :], axis=1)
