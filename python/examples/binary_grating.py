...
def bounded_direction_cosine_acos(direction_cosine):
    """Return acos after accepting only a small FP32 endpoint overshoot."""

    global direction_cosine_max_overshoot
    if not math.isfinite(direction_cosine):
        raise RuntimeError("non-finite eigenmode direction cosine")
    overshoot = max(0.0, abs(direction_cosine) - 1.0)
    direction_cosine_max_overshoot = max(
        direction_cosine_max_overshoot, overshoot
    )
    fp32_endpoint_slop = 32 * np.finfo(np.float32).eps
    if overshoot > fp32_endpoint_slop:
        raise RuntimeError(
            "eigenmode direction cosine exceeds the physical domain by "
            f"{overshoot}, above the FP32 allowance {fp32_endpoint_slop}"
        )
    return math.acos(max(-1.0, min(1.0, direction_cosine)))

for nm in range(nmode):
    for nf in range(nfreq):
        mode_wvl.append(1 / freqs[nf])
        direction_cosine = kdom[nm * nfreq + nf].x / freqs[nf]
        mode_angle.append(
            math.degrees(bounded_direction_cosine_acos(direction_cosine))
        )
