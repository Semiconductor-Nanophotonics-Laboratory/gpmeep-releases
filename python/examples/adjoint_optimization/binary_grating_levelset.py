...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--validation",
        action="store_true",
        help="emit deterministic objective and directional-derivative metrics",
    )
    ...
    one_cell_perturbed_design_weights = levelset_and_smoothing(
        GRATING_HEIGHT_UM + GRATING_HEIGHT_PERTURBATION_UM
    )
    one_cell_perturbed_design_weights = (
        one_cell_perturbed_design_weights.flatten()
    )

    # The level-set primitive changes only on its one-Yee-cell stencil.  A
    # full-cell objective difference includes appreciable second-order
    # electromagnetic response and is not a directional-derivative oracle.
    # Use a small, independently evaluated fraction of that exact density
    # direction so this gate tests the custom VJP and the Maxwell adjoint in
    # their linear regime while retaining a signal well above FP32 noise.
    linearization_fraction = 0.1
    base_design_weights = np.asarray(
        smoothed_design_weights, dtype=np.float64
    ).reshape(-1)
    density_delta = (
        one_cell_perturbed_design_weights - base_design_weights
    )
    perturbed_design_weights = (
        base_design_weights + linearization_fraction * density_delta
    )
    obj_val_perturbed, _ = opt(
        [perturbed_design_weights], need_gradient=False
    )

    adj_directional_deriv = (
        linearization_fraction
        * GRATING_HEIGHT_PERTURBATION_UM
        * grad_backprop
    )
    ...
    np.testing.assert_allclose(
        ...
        atol=2e-7,
        err_msg=(
            "level-set custom VJP/adjoint derivative disagrees with an "
            "independent forward density-direction evaluation"
        ),
    )
    print(
    ...
    if args.validation:
        metrics = {
            "adjoint_directional_derivative": float(
                np.asarray(adj_directional_deriv).reshape(-1)[0]
            ),
            "finite_difference_directional_derivative": float(
                np.asarray(fnd_directional_deriv).reshape(-1)[0]
            ),
            "gradient_l2": float(np.linalg.norm(grad_unperturbed)),
            "linearization_fraction": linearization_fraction,
            "objective_perturbed": float(
                np.asarray(obj_val_perturbed).reshape(-1)[0]
            ),
            "objective_unperturbed": float(
                np.asarray(obj_val_unperturbed).reshape(-1)[0]
            ),
            "relative_derivative_error": float(
                np.asarray(rel_err).reshape(-1)[0]
            ),
        }
