...
def epigraph_constraint(
    ...
    Args:
      ...
      optimization_problem: optional problem override used by deterministic
        validation. The production optimizer uses the module-level ``opt``.
    """
    epigraph = epigraph_and_weights[0]
    ...
    mapping_bias = 0 if use_epsavg else sigmoid_bias
    obj_val, grad = active_opt(
        [
            filter_and_project(
                weights, sigmoid_threshold, mapping_bias
            )
        ]
    )
...
def run_validation() -> None:
    """Evaluate one fixed high-beta mode-converter design."""
    input_flux, input_flux_data = straight_waveguide()
    silicon_mask, silicon_dioxide_mask = border_masks()
    num_weights = NX_DESIGN_GRID * NY_DESIGN_GRID
    weights = np.full(num_weights, 0.5, dtype=np.float64)
    weights[silicon_mask.flatten()] = 1
    weights[silicon_dioxide_mask.flatten()] = 0

    sigmoid_bias = 64
    use_epsavg = True
    validation_opt = mode_converter_optimization(
        input_flux,
        input_flux_data,
        use_damping=True,
    ...
    epigraph = 1.2
    epigraph_and_weights = np.concatenate(([epigraph], weights))
    epigraph_values = np.empty(2 * num_wavelengths)
    epigraph_jacobian = np.empty(
        (2 * num_wavelengths, num_weights + 1)
    )
    epigraph_constraint(
        epigraph_values,
        ...
        sigmoid_bias,
        use_epsavg,
        optimization_problem=validation_opt,
    )
    np.testing.assert_array_equal(
        epigraph_jacobian[:, 0],
        -np.ones(2 * num_wavelengths),
        err_msg="epigraph Jacobian has an invalid dummy-variable column",
    )
    objective = (
        epigraph_values + epigraph
    ).reshape(2, num_wavelengths)
    mapped_gradient = np.empty(
        (2, num_weights, num_wavelengths),
        dtype=np.float64,
    )
    mapped_gradient[0] = epigraph_jacobian[
        :num_wavelengths, 1:
    ].T
    mapped_gradient[1] = epigraph_jacobian[
        num_wavelengths:, 1:
    ].T

    # The published objectives have strongly correlated sensitivities within
    # each six-wavelength family. A normalized oscillatory direction nearly
    # cancels the reflectance gradient and leaves an unusable ~1e-11 FP32
    # signal. Build one well-conditioned direction from each objective
    # family's summed gradient, then verify all six wavelengths with an
    # independent perturbed forward solve.
    fixed_mask = silicon_mask.flatten() | silicon_dioxide_mask.flatten()
    objective_direction = np.sum(mapped_gradient, axis=2)
    objective_direction[:, fixed_mask] = 0
    objective_direction_norm = np.linalg.norm(
        objective_direction, axis=1
    )
    if np.any(objective_direction_norm == 0):
        raise RuntimeError("mode-converter validation produced a zero direction")
    objective_direction /= objective_direction_norm[:, None]
    step = 1e-3
    perturbed_objective = np.empty_like(objective)
    for objective_index in range(objective.shape[0]):
        perturbed_weights = (
            weights + step * objective_direction[objective_index]
        )
        objective_values, _ = validation_opt(
            [
                filter_and_project(
                    perturbed_weights,
                    SIGMOID_THRESHOLD_INTRINSIC,
        ...
        perturbed_objective[objective_index] = np.asarray(
            objective_values, dtype=np.float64
        )[objective_index]

    constraint_values = np.empty(2)
    constraint_gradient = np.empty((2, num_weights + 1))
    ...
    line_gradient = constraint_gradient[:, 1:]
    line_direction_norm = np.linalg.norm(line_gradient, axis=1)
    if np.any(line_direction_norm == 0):
        raise RuntimeError("line constraint produced a zero gradient")
    line_direction = line_gradient / line_direction_norm[:, None]
    line_step = 1e-3
    perturbed_constraint_values = np.empty(2)
    for constraint_index in range(2):
        candidate_values = np.empty(2)
        candidate_gradient = np.empty((2, num_weights + 1))
        line_width_and_spacing_constraint(
            candidate_values,
            np.concatenate(
                ([1.2], weights + line_step * line_direction[constraint_index])
            ),
            candidate_gradient,
            sigmoid_bias,
        )
        perturbed_constraint_values[constraint_index] = (
            candidate_values[constraint_index]
        )
    objective_adjoint_directional = (
        step
        * np.einsum(
            "oi,oif->of", objective_direction, mapped_gradient
        )
    )
    objective_finite_difference = perturbed_objective - objective
    line_adjoint_directional = (
        line_step
        * np.einsum("oi,oi->o", line_direction, line_gradient)
    )
    line_finite_difference = (
        perturbed_constraint_values - constraint_values
    )
    np.testing.assert_allclose(
        ...
        rtol=0.05,
        atol=1e-6,
        err_msg="mode-converter mapped gradient disagrees with forward differences",
    )
    np.testing.assert_allclose(
        line_adjoint_directional,
        line_finite_difference,
        rtol=0.01,
        atol=1e-8,
        err_msg="line-constraint gradient disagrees with forward differences",
    )

    metrics = {
        "adjoint_directional_derivative": (
            objective_adjoint_directional.tolist()
        ),
        "finite_difference_directional_derivative": (
            objective_finite_difference.tolist()
        ),
        "gradient_l2": np.linalg.norm(
            mapped_gradient, axis=1
        ).tolist(),
        "input_flux": np.asarray(input_flux, dtype=np.float64).tolist(),
        "epigraph_constraint": epigraph_values.tolist(),
        "epigraph_jacobian_dummy_column": (
            epigraph_jacobian[:, 0].tolist()
        ),
        "line_constraint": constraint_values.tolist(),
        "line_constraint_adjoint_directional_derivative": (
            line_adjoint_directional.tolist()
        ),
        "line_constraint_finite_difference_directional_derivative": (
            line_finite_difference.tolist()
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--validation",
        action="store_true",
        help="run one fixed mode-converter design with gradient/FD metrics",
    )
