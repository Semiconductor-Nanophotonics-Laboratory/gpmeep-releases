...
Rfresnel_value = Rfresnel(theta_r)
Rmeep_values = [Rmeep_real_imag, Rmeep_complex, Rmeep_3d]
analytic_relative_errors = [
    abs(value - Rfresnel_value) / abs(Rfresnel_value) for value in Rmeep_values
]
representation_spread = max(Rmeep_values) - min(Rmeep_values)

print(
    f"refl:, {Rmeep_real_imag} (real/imag), {Rmeep_complex} (complex), {Rmeep_3d} (3d), {Rfresnel_value} (analytic)"
)
