...
parser.add_argument(
    "--validation",
    action="store_true",
    help="run a reduced physical case and emit CPU/CUDA comparison metrics",
)
...
# Cavity definitions
resolution = 32 if args.validation else 400
...
endt = 12 if args.validation else 7000
...
if args.validation:
    sim.run(until=endt)
    sample_positions = (-1.25, -1.0, -0.75, -0.25, 0.5)
    metrics = {
        "ex": [
            float(
                sim.get_field_point(
