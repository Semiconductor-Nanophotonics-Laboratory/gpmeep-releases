...
size_parameters = 2 * np.pi * r * np.asarray(freqs)
scatt_negative_violation = max(0.0, -float(np.min(scatt_cross_section)))

if mp.am_master() and not args.validation:
    plt.figure(dpi=150)
    plt.loglog(size_parameters, scatt_cross_section, "bo-")
