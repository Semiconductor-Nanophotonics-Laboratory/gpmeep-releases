...
far_field_amplitudes = np.empty((0, 6), dtype=np.complex128)
...
def radiation_pattern(sim: mp.Simulation, n2f_mon: mp.DftNear2Far) -> np.ndarray:
    ...
    far_field_amplitudes = np.column_stack((e_field, h_field))
...
def antenna_above_ground_plane(
    dipole_polarization: int, *, make_plots: bool = True
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    radial_flux_meep_raw = antenna_above_ground_plane(
        dipole_polarization, make_plots=not args.validation
    )
    radial_flux_normalization = np.max(radial_flux_meep_raw)
    # Normalize radiation pattern to lie in range [0, 1].
    radial_flux_meep = radial_flux_meep_raw / radial_flux_normalization
