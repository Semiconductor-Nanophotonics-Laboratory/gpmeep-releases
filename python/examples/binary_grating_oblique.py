...
Rsum = 0
reflected_mode_power = []
reflected_mode_angle = []
for nm in range(nm_r):
    r_kdom = res.kdom[nm]
    Rmode = abs(r_coeffs[nm, 0, 1]) ** 2 / input_flux[0]
    r_angle = np.sign(r_kdom.y) * math.acos(r_kdom.x / (ng * fcen))
    reflected_mode_power.append(Rmode)
    reflected_mode_angle.append(math.degrees(r_angle))
...
Tsum = 0
transmitted_mode_power = []
transmitted_mode_angle = []
for nm in range(nm_t):
    t_kdom = res.kdom[nm]
    Tmode = abs(t_coeffs[nm, 0, 0]) ** 2 / input_flux[0]
    t_angle = np.sign(t_kdom.y) * math.acos(t_kdom.x / fcen)
    transmitted_mode_power.append(Tmode)
    transmitted_mode_angle.append(math.degrees(t_angle))
...
mode_flux_totals = [Rsum, Tsum, Rsum + Tsum]
poynting_flux_totals = [Rflux, Tflux, Rflux + Tflux]
mode_poynting_disagreement = [Rsum - Rflux, Tsum - Tflux]
mode_energy_balance_error = abs(Rsum + Tsum - 1)
poynting_energy_balance_error = abs(Rflux + Tflux - 1)
