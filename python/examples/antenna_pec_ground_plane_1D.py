...
if __name__ == "__main__":
    ...
    sampled_mask = np.zeros(NUM_POLAR, dtype=bool)
    ...
    antenna_1d_radial_flux_raw = np.asarray(radial_flux_meep.copy())
    antenna_1d_peak_radial_flux = np.asarray([radial_flux_meep_max])
    radial_flux_meep = radial_flux_meep / radial_flux_meep_max

    antenna_1d_polar_angles = np.asarray(polar_rad)
    antenna_1d_sampled_mask = sampled_mask.astype(float)
    antenna_1d_meep_pattern = np.asarray(radial_flux_meep)
    antenna_1d_analytic_pattern = np.asarray(radial_flux_analytic)
    antenna_1d_relative_error = np.asarray(
        [
            np.linalg.norm(
                antenna_1d_meep_pattern[sampled_mask]
                - antenna_1d_analytic_pattern[sampled_mask]
            )
            / np.linalg.norm(antenna_1d_analytic_pattern[sampled_mask])
        ]
    )
    antenna_1d_peak_angle = np.asarray([polar_rad_max])

    rel_err = antenna_1d_relative_error[0]
    print(f"error:, {rel_err:.6f}")

    if args.validation:
        retained = (
            antenna_1d_polar_angles,
            antenna_1d_sampled_mask,
            antenna_1d_radial_flux_raw,
            antenna_1d_peak_radial_flux,
            antenna_1d_meep_pattern,
            antenna_1d_analytic_pattern,
            antenna_1d_relative_error,
            antenna_1d_peak_angle,
        )
        if not all(np.all(np.isfinite(values)) for values in retained):
            raise RuntimeError("antenna 1D validation produced nonfinite data")
        if np.count_nonzero(sampled_mask) < 5:
            raise RuntimeError("antenna 1D validation sampled too few angles")
        if antenna_1d_peak_radial_flux[0] <= 0:
            raise RuntimeError("antenna 1D peak flux is not outward and positive")
        if np.any(antenna_1d_radial_flux_raw[sampled_mask] < 0):
            raise RuntimeError("antenna 1D sampled an inward radial flux")
        if np.max(antenna_1d_meep_pattern[sampled_mask]) < 0.5:
            raise RuntimeError("antenna 1D radiation pattern has no resolved lobe")
        if antenna_1d_relative_error[0] > 0.08:
            raise RuntimeError(
                "antenna 1D pattern disagrees with the two-element array law: "
                f"{antenna_1d_relative_error[0]}"
            )
