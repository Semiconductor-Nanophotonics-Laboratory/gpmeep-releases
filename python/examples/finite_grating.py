...
expected_scattered_field_shape = (501, 352)
if scattered_field.shape != expected_scattered_field_shape:
    raise RuntimeError(
        f"unexpected scattered-field shape {scattered_field.shape}; "
        f"expected {expected_scattered_field_shape}"
    )
scattered_amplitude = np.abs(scattered_field) ** 2
scattered_field_flat = np.asarray(scattered_field).reshape(-1)
scattered_sample_indices = np.linspace(
    0, scattered_field_flat.size - 1, 257, dtype=int
)
scattered_field_samples = scattered_field_flat[scattered_sample_indices]
scattered_amplitude_samples = np.abs(scattered_field_samples) ** 2
scattered_field_shape = np.asarray(scattered_field.shape, dtype=int)
scattered_field_full_norm_max = np.asarray(
    [
        np.linalg.norm(scattered_field_flat),
        np.max(np.abs(scattered_field_flat)),
    ]
)
