...
if (
    not np.isfinite(absorbed_power)
    or not np.isfinite(absorbed_flux)
    or absorbed_power <= 0
    or absorbed_flux <= 0
):
    raise RuntimeError(
        "passive SiO2 absorption must have positive finite volume and "
        f"surface powers: volume={absorbed_power}, surface={absorbed_flux}"
    )
err = abs(absorbed_power - absorbed_flux) / abs(absorbed_flux)
print(
    f"flux:, {absorbed_power} (dft_fields), {absorbed_flux} (dft_flux), {err} (error)"
)
if not np.isfinite(err) or err >= 0.02:
    raise RuntimeError(
        "absorbed-power volume integral and closed-surface flux disagree: "
