...
import meep as mp
try:
    from meep.mie_theory import mie_scattering_efficiency
except ModuleNotFoundError as exc:
    if exc.name != "meep.mie_theory":
        raise
    from mie_theory import mie_scattering_efficiency
...
parser.add_argument(
    "--validation",
    action="store_true",
    help="run a bounded deterministic Mie-scattering oracle",
)
...
wvl_min = 2 * np.pi * r / (6 if args.validation else 10)
wvl_max = 2 * np.pi * r / 2
...
nfrq = 12 if args.validation else 100

## at least 8 pixels per smallest wavelength, i.e. np.floor(8/wvl_min)
resolution = 20 if args.validation else 25
...
scatt_eff_theory = mie_scattering_efficiency(
    n_sphere, 2 * np.pi * r * np.asarray(freqs)
)

if args.validation:
    relative_l2 = float(
        np.linalg.norm(scatt_eff_meep - scatt_eff_theory)
        / np.linalg.norm(scatt_eff_theory)
    )
    scale_relative_max = float(
        np.max(np.abs(scatt_eff_meep - scatt_eff_theory))
        / np.max(np.abs(scatt_eff_theory))
    )
    values = np.concatenate(
        (
            np.asarray(freqs),
            np.asarray(scatt_eff_meep),
            np.asarray(scatt_eff_theory),
        )
    )
    if (
        not np.all(np.isfinite(values))
        or not np.all(np.asarray(scatt_eff_meep) > 0)
        or relative_l2 > 0.15
        or scale_relative_max > 0.15
    ):
        raise RuntimeError("bounded Mie scattering validation failed")
    metrics = {
        "schema_version": 1,
        "resolution": resolution,
        "frequency_count": nfrq,
        "size_parameters": (2 * np.pi * r * np.asarray(freqs)).tolist(),
        "meep_efficiency": np.asarray(scatt_eff_meep).tolist(),
        "theory_efficiency": np.asarray(scatt_eff_theory).tolist(),
        "relative_l2": relative_l2,
        "scale_relative_max": scale_relative_max,
    }
    if mp.am_master():
        print(
            "gpmeep-mie-scattering-metrics:"
