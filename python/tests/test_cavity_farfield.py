...
class TestCavityFarfield(ApproxComparisonTestCase):
    ...
    def run_test(self, nfreqs):
        ...
        batched = sim.get_farfields_points(nearfield, arbitrary_points)
        self.assertEqual(batched.shape, (len(arbitrary_points), nfreqs, 6))
        for point_index, point in enumerate(arbitrary_points):
            scalar = sim.get_farfield(nearfield, point)
            self.assertClose(
                np.asarray(scalar).reshape(nfreqs, 6),
                batched[point_index],
                epsilon=1e-12,
            )
