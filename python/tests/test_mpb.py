...
# The packaged MPB library is non-MPI and each Meep MPI rank runs an
# independent eigensolve.  Threaded BLAS/OpenMP reduction order can otherwise
# select measurably different eigenvectors on different ranks even with
# ModeSolver(deterministic=True), making the checked-in field oracle flaky.
# Constrain this test process only; production MPB/Meep settings are untouched.
...
class TestModeSolver(ApproxComparisonTestCase):
    ...
    def compare_complex_h5_field(self, ref_path, res_path, component, tol=1e-3):
        real_key = f"{component}.r"
        imag_key = f"{component}.i"
        with h5py.File(ref_path) as ref:
            with h5py.File(res_path, "r") as res:
                self.assertEqual(set(ref.keys()), set(res.keys()))
                for key in set(ref.keys()) - {real_key, imag_key}:
                    if key == "description":
                        self.assertEqual(ref[key][()], res[key][()])
                    else:
                        self.assertClose(ref[key][()], res[key][()], epsilon=tol)
                reference_field = mp.complexarray(
                    ref[real_key][()], ref[imag_key][()]
                )
                result_field = mp.complexarray(
                    res[real_key][()], res[imag_key][()]
                )
                self.assertClose(reference_field, result_field, epsilon=tol)
    ...
    def test_output_efield_z(self):
        ...
        # Phase fixing makes the imaginary channel nearly zero.  Comparing the
        # real and imaginary residuals independently turns harmless solver
        # phase noise into an arbitrarily large relative error.  The physical
        # oracle is the complete complex field.
        self.compare_complex_h5_field(ref_path, res_path, "z")
