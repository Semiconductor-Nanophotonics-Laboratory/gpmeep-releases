...
class FilteredSource(CustomSource):
    ...
    def estimate_impulse_response(self, H):
        ...
        # Use a global relative residual.  A sparse objective commonly asks
        # for exactly zero response at most monitor entries; elementwise
        # normalization would divide those valid zeros by zero and make this
        # diagnostic NaN even when the fitted source is well conditioned.
        residual_energy = np.sum(np.abs(H - H_hat.T) ** 2)
        response_energy = np.sum(np.abs(H) ** 2)
        l2_err = residual_energy / max(response_energy, np.finfo(float).tiny)
        return nodes, l2_err
