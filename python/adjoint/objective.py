...
def _filtered_source(*args, **kwargs):
    """Construct a multi-frequency source without eager SciPy imports."""

    from .filter_source import FilteredSource

    return FilteredSource(*args, **kwargs)
...
class Near2FarFields(ObjectiveQuantity):
    ...
    def __init__(
        ...
        Args:
          ...
          norm_near_fields: the DFT fields obtained using `get_near2far_data` from
            ...
            Default is None.
          cartesian: for a cylindrical simulation, preserve `far_pts` as
            arbitrary global xyz points and use global Cartesian E/H
            components. The default keeps the legacy phi=0 cylindrical
            interpretation.
        """
        super().__init__(sim)
    ...
    def place_adjoint_source(self, dJ):
        ...
        far_pt_vec = py_v3_to_vec(
            3 if self.cartesian and self.sim.is_cylindrical
            else self.sim.dimensions,
            far_pt0,
            self.sim.is_cylindrical and not self.cartesian,
        )

        all_nearsrcdata = self._monitor.swigobj.near_sourcedata(
            far_pt_vec, farpt_list, self._nfar_pts, dJ, self.greencyl_tol
        )
        adjoint_scale = self._adj_src_scale(include_resolution=False)
        if (
            self.sim._infer_dimensions(self.sim.k_point) == 3
            and self.num_freq == 1
        ):
            # near_sourcedata supplies the physical surface-quadrature weight,
            # but the single-frequency 3D IndexedSource also needs the Yee-cell
            # volume that maps its discrete current back to the MaterialGrid
            # volume inner product.  Its one-sided Wirtinger contraction adds
            # the factor of one half.  Multi-frequency sources use the existing
            # FilteredSource frequency-response normalization instead; applying
            # this single-frequency correction there double-counts its scale.
            adjoint_scale = 0.5 * self._adj_src_scale(include_resolution=True)
    ...
    def __call__(self):
        ...
        """
        self._eval = np.asarray(
            self.sim.get_farfields_points(
                self._monitor,
                self.far_pts,
                self.greencyl_tol,
