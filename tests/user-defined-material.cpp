...
static void test_material_thread_safety_classification(meep::structure &s) {
  geometric_object_list geometry = {0, 0};
  vector3 center = {0, 0, 0};
  meep_geom::material_type materials[] = {
      meep_geom::make_dielectric(2.0),
      meep_geom::make_user_material(my_material_func, nullptr, false),
      meep_geom::make_material_grid(true, 0.0, 0.5, 0.0),
      new meep_geom::material_data(),
  };
  materials[3]->which_subclass = meep_geom::material_data::MATERIAL_FILE;
  const bool expected_thread_safe[] = {false, false, false, false};

  for (size_t i = 0; i < sizeof(materials) / sizeof(materials[0]); ++i) {
    meep_geom::geom_epsilon *geps =
        meep_geom::make_geom_epsilon(&s, &geometry, center, false, materials[i]);
    if (geps->is_thread_safe() != expected_thread_safe[i])
      meep::abort("incorrect material thread-safety classification at index %zu\n", i);
    delete geps;
    meep_geom::material_free(materials[i]);
  }
  meep_geom::unset_default_material();
}

static void test_zero_subnormals_all_threads() {
#if defined(_OPENMP) && HAVE_IMMINTRIN_H
  const int original_dynamic = omp_get_dynamic();
  const int original_max_threads = omp_get_max_threads();
  omp_set_dynamic(0);
  omp_set_num_threads(4);

  const unsigned int flags = 0x00008040;
  meep::set_zero_subnormals(false);
  meep::set_zero_subnormals(true);
  int enabled = 0;
  int thread_count = 0;
#pragma omp parallel reduction(+ : enabled, thread_count)
  {
    ++thread_count;
    enabled += (_mm_getcsr() & flags) == flags;
  }
  if (enabled != thread_count)
    meep::abort("flush-to-zero/denormals-are-zero was not enabled on every OpenMP thread\n");

  meep::set_zero_subnormals(false);
  int disabled = 0;
#pragma omp parallel reduction(+ : disabled)
  { disabled += (_mm_getcsr() & flags) == 0; }
  if (disabled != thread_count)
    meep::abort("flush-to-zero/denormals-are-zero was not disabled on every OpenMP thread\n");
...
int main(int argc, char *argv[]) {
  ...
  test_zero_subnormals_all_threads();
  ...
  test_material_thread_safety_classification(the_structure);
