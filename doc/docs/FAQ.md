...
The field from a point source is singular &mdash; it blows up as you approach the source. At any finite grid resolution, this singularity is truncated to a finite value by the grid discretization but the peak field at the source location increases as you increase the resolution.
...
As another possibility, if an *entire* boundary consists of a material where $\varepsilon$ has a negative real part (and $\mu$ is positive), or is sufficiently lossy, then realize that you don't need a PML at all — all waves are exponentially decaying in such a material, so you just need to make the material sufficiently thick at the boundary.
...
The [effective index](https://www.rp-photonics.com/effective_refractive_index.html) is usually defined as $n_{eff} = \beta / \omega$.

To compute the (complex) effective index of a lossy waveguide, you will need to first compute the *complex* $\omega$ (the loss in time) for a *real* $\beta$ (the propagation constant) and then convert this quantity into a loss in space (*complex* $\beta$ at a *real* $\omega$) by dividing by the group velocity $v_g$. This procedure is described in more detail below.
...
For a *lossless* waveguide, $\beta$ and $\omega$ are purely real quantities. This means you do not have to worry about group velocity and imaginary parts. You can also compute $\omega(\beta)$ (more efficiently) using the mode solver [MPB](https://mpb.readthedocs.io/en/latest/).
...
There are at least seven ways to define a structure: (1) the [`GeometricObject`](Python_User_Interface.md#geometricobject) (Python) or [`geometric-object`](Scheme_User_Interface.md#geometric-object) (Scheme) class used to specify a collection of predefined shapes including `Mesh`, `Prism`, `Sphere`, `Cylinder`, `Cone`, `Block`, and `Ellipsoid`, (2) [`material_function`](Python_User_Interface.md#medium) (Python) or [`material-function`](Scheme_User_Interface.md#material-function) (Scheme) used to define an arbitrary function: for a given position in the cell, return the $\varepsilon$/$\mu$ at that point, (3) import planar geometries from a [GDS file](Python_Tutorials/GDS_Import.md), (4) import 3d geometries from a triangular mesh (into a `Mesh` object), or [`meep-structure-load`](Scheme_User_Interface.md#load-and-dump-structure) (Scheme), or (5) a [`MaterialGrid`](Python_User_Interface.md#materialgrid) used to specify a pixel grid, (6) import the scalar, real-valued, frequency-independent permittivity from an HDF5 file (which can be created using e.g., [h5py](http://docs.h5py.org/en/stable/)) via the `epsilon_input_file` (Python) or `epsilon-input-file` (Scheme) input parameter, or (7) load the raw $\varepsilon$/$\mu$ saved from a previous simulation using [`load_structure`](Python_User_Interface.md#load-and-dump-structure) (Python). Combinations of (1–6) are allowed, but not (7).
...
The [discrete time Fourier transform](https://en.wikipedia.org/wiki/Discrete-time_Fourier_transform) (DTFT) of the fields, which is necessary for computing the [Poynting flux](Python_User_Interface.md#flux-spectra), [local density of states](Python_User_Interface.md#ldos-spectra) (LDOS), [near to far field transformation](Python_User_Interface.md#near-to-far-field-spectra), etc., is typically accumulated at every time step for every point in the [FluxRegion](Python_User_Interface.md#fluxregion). The DTFT computation is parallelized but only in the sense that each processor computes the DTFT fields at points in its own [chunk](Chunks_and_Symmetry.md) of the grid. If the division of the grid among processors into approximately equal-sized chunks (which is the default specified by `split_chunks_evenly=True`) allocates most of the points where the DTFT fields are computed to one processor, it is *not* going to parallelize.

The DTFT computation can be sped up significantly using decimation of the time-series inputs. By default, this feature is enabled for all monitors (using the parameter `decimation_factor=0`). The optimal decimation factor is chosen automatically based on the frequency content of the sources and monitors.
...
In general, calculating the computational cost of an arbitrary simulation is challenging given the variety of computational elements involved. A simple lower bound of the memory requirements can be obtained for an *empty* cell (i.e., without scalar dielectric materials, [PMLs](Python_User_Interface.md#pml), [DFT monitors](Python_User_Interface.md#flux-spectra), etc.) with Cartesian dimensions $s_x \times s_y \times s_z$ and grid resolution $r$ containing only real fields as: $96 s_x s_y s_z r^3$ bytes. In this case which involves a single [chunk](Chunks_and_Symmetry.md#chunk-data-structures), each grid voxel stores 9 electromagnetic field components (*E*,*D*,*H* in three directions) as well as 3 scalar permittivity values (diagonal entries of the ε tensor) using [double-precision floating point](https://en.wikipedia.org/wiki/Double-precision_floating-point_format) (8 bytes per number). (For the default case of nonmagnetic materials, $\mu=1$ and $B=H$ are not stored.) As an example, a 10×20×30 cell with resolution 50 pixels/unit would require at least 72 GB of memory. Note: doubling the grid resolution would increase the memory requirements by a factor of at least 8 *and* the number of timesteps by exactly 2 (given that the Courant factor and runtime are fixed). Additional storage (and [floating-point operations](https://en.wikipedia.org/wiki/FLOPS) per time step) is required for e.g. dispersive materials based on [Drude-Lorentz susceptibilities](Python_User_Interface.md#susceptibility) and PMLs which involve auxiliary fields, [subpixel smoothing](Subpixel_Smoothing.md) which introduces anisotropic $\varepsilon$ tensors at interface voxels, DFT fields for each frequency bin and field component necessary to compute the Poynting vector, etc. The actual memory requirement may therefore be significantly larger than this lower bound.

### Can Meep be compiled to run on graphics processing units (GPUs)?

Upstream Meep does not currently provide a GPU backend. This gpmeep fork adds
an experimental FP32 NVIDIA CUDA backend covering the core FDTD step, sources,
boundaries, dispersive polarizations, DFT accumulation, adjoint material
gradients, and MPI execution. Use `MEEP_GPU_BACKEND=auto` for workload-aware
CPU/CUDA selection or `MEEP_GPU_BACKEND=cuda` for strict CUDA qualification.
See [Meep GPU Development](../../GPU_DEVELOPMENT.md) for the supported build,
runtime, portability, numerical-validation, and remaining host-only feature
boundaries. OpenCL and HIP backends are not implemented.
...
Visualization in 3d can be done with [Mayavi](http://docs.enthought.com/mayavi/mayavi/index.html). For an example, see [Tutorial/Basics/Visualizing 3d Structures](Python_Tutorials/Basics.md#visualizing-3d-structures).

To output the data to an HDF5 file, you can use the [`in_volume`](Python_User_Interface.md#modifying-hdf5-output) or `in_point` routines as part of your [run function](Python_User_Interface.md#run-functions). For example, to restrict the output to a line, you could use: `meep.in_volume(meep.Volume(center=meep.Vector3(0,0,0), size=meep.Vector3(10,0,0)), meep.output_dpwr)` which outputs $\varepsilon|E|^2$ along a line of length 10 in the x direction centered at (0,0,0). You can even wrap this statement in `to_appended("line.h5", ...)` to output the intensity along the line as a function of time to a 2d HDF5 dataset. This would enable you to plot intensity vs. time and space as a 2d color image.
