...
By default, the volume specified in `get_farfields` via `center` and `size` is in cylindrical coordinates. These points must therefore lie in the $\phi = 0$ ($rz = xz$) plane. The fields $E$ and $H$ returned by `get_farfields` can be thought of as either cylindrical ($r$,$\phi$,$z$) or Cartesian ($x$,$y$,$z$) coordinates since these are the same in the $\phi = 0$ plane (i.e., $E_r=E_x$ and $E_\phi=E_y$).

To evaluate a cylindrical near-field monitor at arbitrary three-dimensional observation points, pass `cartesian=True` to `get_farfield`, `get_farfields_points`, `get_farfields`, or `output_farfields`. In that mode, all $(x,y,z)$ coordinates are preserved and the returned or written components are the global Cartesian $(E_x,E_y,E_z,H_x,H_y,H_z)$ components. The FDTD simulation and near-field monitor remain cylindrical; only the observation geometry changes.

`get_farfields` tends to gradually *slow down* as the far-field point gets closer to the near-field monitor. This performance degradation is unavoidable and is due to the larger number of $\phi$ integration points required for accurate convergence of the integral involving the Green's function which diverges as the evaluation point approaches the source point.
...
3. Compute the radiated power within an angular cone $\int_0^\theta \int_0^{2\pi} P(\theta') \sin(\theta') d\phi d\theta'$ by some discretized integral, e.g. a [trapezoidal rule](https://en.wikipedia.org/wiki/Trapezoidal_rule). The extraction efficiency involves dividing this quantity by the total power emitted by the dipole.  Dividing by the total radiated power $\int_0^{\pi/2} \int_0^{2\pi} P(\theta') \sin(\theta') d\phi d\theta'$ yields the collection efficiency (which differs from the extraction efficiency if there is absorption).
