...
static int nextpow2357(int n) {
  ...
  while (candidate <= std::numeric_limits<int>::max()) {
    long long m = candidate;
    while (m % 3 == 0)
      m /= 3;
    while (m % 5 == 0)
      m /= 5;
  ...
  meep::abort("eigenmode grid dimension is too large");
}
...
void *fields::get_eigenmode(double frequency, direction d, const volume where, const volume eig_vol,
  ...
  direction dp_normal = NO_DIRECTION;
  double match_tol;

  if (band_num <= 0) meep::abort("eigenmode band index must be positive");
  if ((!dp && d == NO_DIRECTION && abs(_kpoint) == 0) || coordinate_mismatch(gv.dim, d))
  ...
  // DiffractedPlanewave input errors are catchable Python exceptions.  Validate them before any
  // MPB storage is allocated so a rejected source cannot leak mdata or eigensolver work arrays.
  if (dp) {
    dp_normal = d == NO_DIRECTION ? where.normal_direction() : d;
    if (dp_normal < X || dp_normal > Z || coordinate_mismatch(gv.dim, dp_normal))
      meep::abort("cannot determine a valid propagation direction for DiffractedPlanewave");
    if (dp->get_g()[dp_normal - X] != 0)
      meep::abort("DiffractedPlanewave reciprocal order must be parallel to its source plane");
  }
  ...
  double transverse_kcart_len = sqrt(kcart[0] * kcart[0] + kcart[1] * kcart[1]);
  for (int i = 0; i < 3; ++i)
    if (!std::isfinite(s[i]) || s[i] < 0.0 || !std::isfinite(o[i]))
      meep::abort("non-finite or negative eigenmode volume extent");
  if (!std::isfinite(resolution) || resolution <= 0.0)
    meep::abort("non-finite or non-positive eigenmode resolution");
  if (!std::isfinite(frequency) || (match_frequency && frequency <= 0.0) ||
      !std::isfinite(eigensolver_tol) || eigensolver_tol <= 0.0 || !std::isfinite(kcart_len) ||
      !std::isfinite(transverse_kcart_len))
    meep::abort("non-finite or non-positive eigenmode frequency, tolerance, or wavevector");
  if (!dp && d == NO_DIRECTION && kcart_len == 0.0)
    meep::abort("NO_DIRECTION requires a nonzero wavevector in the simulation dimensions");
  match_tol = eigensolver_tol * 10;
  if (!std::isfinite(match_tol)) meep::abort("eigenmode matching tolerance overflow");

  for (int i = 0; i < 3; ++i) {
    double requested_grid_size = resolution * s[i] + 0.5;
    if (!std::isfinite(requested_grid_size) ||
        requested_grid_size > std::numeric_limits<int>::max())
      meep::abort("eigenmode grid dimension is too large");
    n[i] = int(requested_grid_size);
    if (n[i] == 0) n[i] = 1;
    n[i] = nextpow2357(n[i]);
    if (s[i] != 0)
      R[i][i] = s[i];
    else {
      // DiffractedPlanewave has an independently inferred propagation normal.  Its zero-width
      // lattice axis must remain that Cartesian normal even when the Bloch vector is zero or
      // oblique; aligning it with kparallel makes it parallel to an in-plane lattice row.
      if (d != NO_DIRECTION || dp || empty_dim[i])
        R[i][i] = 1;
      else if (gv.dim == D2) {
        // In special-kz mode beta is fixed and is not part of the transverse propagation
        // direction used to construct a zero-width eigenmode lattice axis.  Including beta here
        // can make R singular at normal incidence because both this row and the empty z row become
        // parallel to z.
        if (transverse_kcart_len > 0.0) {
          R[i][0] = kcart[0] / transverse_kcart_len;
          R[i][1] = kcart[1] / transverse_kcart_len;
        }
        else
          R[i][i] = 1;
      }
  ...
  int eigenmode_grid_points = 1;
  for (int i = 0; i < 3; ++i) {
    if (n[i] > std::numeric_limits<int>::max() / eigenmode_grid_points)
      meep::abort("eigenmode grid point count is too large");
    eigenmode_grid_points *= n[i];
  }

  if (dp) {
    bool dp_input_valid = true;
    for (int i = 0; i < 3; ++i) {
      // maxwell_set_planewave maps a requested reciprocal order to an FFT index and uses a
      // fatal CHECK when the index is outside the grid.  It also accepts some aliased orders
      // which select a different physical G than the one used in our analytic dispersion.
      // Require the exact, non-aliased order before entering MPB.
      const long long order = static_cast<long long>(dp->get_g()[i]);
      const long long fft_index = order > 0 ? static_cast<long long>(n[i]) - order : -order;
      if (fft_index < 0 || fft_index >= static_cast<long long>(n[i])) {
        dp_input_valid = false;
        break;
      }
      const long long half_grid = static_cast<long long>(n[i]) / 2;
      const long long center = half_grid > 1 ? half_grid : 1;
      const long long signed_index =
          fft_index >= center ? fft_index - static_cast<long long>(n[i]) : fft_index;
      if (-signed_index != order) {
        dp_input_valid = false;
        break;
      }
    }
    const complex<double> dp_s = dp->get_s();
    const complex<double> dp_p = dp->get_p();
    double axis_norm2 = 0.0;
    for (int i = 0; i < 3; ++i) {
      const double axis_i = dp->get_axis()[i];
      if (!std::isfinite(axis_i)) dp_input_valid = false;
      axis_norm2 += axis_i * axis_i;
    }
    if (!std::isfinite(axis_norm2) || axis_norm2 <= 0.0 ||
        !std::isfinite(real(dp_s)) || !std::isfinite(imag(dp_s)) ||
        !std::isfinite(real(dp_p)) || !std::isfinite(imag(dp_p)) ||
        (dp_s == complex<double>(0.0, 0.0) && dp_p == complex<double>(0.0, 0.0)))
      dp_input_valid = false;
    if (!dp_input_valid) {
      if (am_master())
        master_printf("WARNING: invalid or unrepresentable DiffractedPlanewave input for "
                      "g=(%d,%d,%d)\n",
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (!std::isfinite(static_cast<double>(k[i])))
      meep::abort("non-finite eigenmode wavevector in reciprocal basis");
    // G = inverse of R transpose, via cross-product formula
    cross_product(G[i], R[(i + 1) % 3], R[(i + 2) % 3]);
    double GdotR = dot_product(G[i], R[i]);
    if (!std::isfinite(GdotR) || GdotR == 0.0)
      meep::abort("singular eigenmode lattice basis; the mode wavevector may be tangent to a "
                  "zero-width eig_vol axis");
    for (int j = 0; j < 3; ++j) {
      G[i][j] /= GdotR;
      if (!std::isfinite(static_cast<double>(G[i][j])))
        meep::abort("non-finite reciprocal eigenmode lattice basis");
    }
  }
  ...
  if (d == NO_DIRECTION) {
    if (gv.dim == D2) {
      kmatch = transverse_kcart_len;
      // beta is fixed in a 2d special-kz solve, so Newton matching changes only the transverse
      // wavevector.  Normalize that transverse direction independently of beta; normalizing by
      // the full 3d |k| and then zeroing z would make kdir non-unit and change k even for a zero
      // Newton step.
      if (kmatch > 0.0) {
        kdir[0] = kcart[0] / kmatch;
        kdir[1] = kcart[1] / kmatch;
      }
      else
        // Prefer the zero-width monitor axis as the propagation direction.  If neither axis is
        // zero width, X is the deterministic fallback.
        kdir[eig_vol.in_direction(X) == 0.0
                 ? 0
                 : (eig_vol.in_direction(Y) == 0.0 ? 1 : 0)] = 1.0;
      kdir[2] = 0;
    }
    else {
      kmatch = kcart_len;
  ...
  if (!dp && match_frequency && kmatch == 0) {
    vec cen = eig_vol.center();
    double eps_mu = real(get_eps(cen, frequency)) * real(get_mu(cen, frequency));
    if (!std::isfinite(eps_mu) || eps_mu <= 0.0)
      meep::abort("non-finite or non-positive epsilon*mu in automatic eigenmode k guess");
    kmatch = frequency * sqrt(eps_mu);
    if (!std::isfinite(kmatch) || kmatch <= 0.0)
      meep::abort("non-finite or non-positive automatic eigenmode k guess");
    if (d == NO_DIRECTION) {
      for (int i = 0; i < 3; ++i) {
        // Convert the physical Cartesian target back to MPB's reciprocal basis.  In 2d beta is
        // fixed, and its R[i][2]*beta contribution must be retained for non-diagonal R.
        k[i] = dot_product(R[i], kdir) * kmatch;
        if (gv.dim == D2) k[i] += R[i][2] * beta;
      }
    }
    else {
      ...
      // The Brillouin-zone clamp changes the actual physical starting point; keep Newton's scalar
      // anchor synchronized with the k passed to MPB.
      kmatch = G[d - X][d - X] * k[d - X];
    }
    for (int i = 0; i < 3; ++i)
      if (!std::isfinite(static_cast<double>(k[i])))
        meep::abort("non-finite reciprocal wavevector from automatic eigenmode k guess");
  ...
  if (!user_mdata || *user_mdata == NULL) {
    ...
    if (local_N != eigenmode_grid_points) {
      destroy_maxwell_data(mdata);
      meep::abort("MPI version of MPB library is not supported");
    }
  ...
  // Invalidate an externally cached allocation as well as direct-call storage when input or
  // material validation fails.  This keeps Python exceptions recoverable without retaining a
  // half-initialized MPB object in the monitor cache.
  auto invalidate_mdata = [&]() {
    if (user_mdata) {
      if (*user_mdata == mdata) {
        *user_mdata = NULL;
        destroy_maxwell_data(mdata);
  ...
  if (!dp && k[0] == 0 && k[1] == 0 && k[2] == 0) {
    ...
    if (band_num == 0) {
      invalidate_mdata();
      meep::abort("zero-frequency bands at k=0 are ill-defined");
    }
  }

  evectmatrix H = create_evectmatrix(eigenmode_grid_points, 2, band_num, local_N, N_start, alloc_N);
  ...
  evectmatrix W[3];
  for (int i = 0; i < 3; ++i)
    W[i] = create_evectmatrix(eigenmode_grid_points, 2, band_num, local_N, N_start, alloc_N);
  ...
  mpb_real vgrp = 0.0; // Re( W[0]* (dTheta/dk) W[0] ) = group velocity
  bool frequency_converged = !match_frequency;
  int newton_solve_count = 0;
  const int max_newton_solves = 50;
  ...
  /*--------------------------------------------------------------*/
  if (am_master() && !dp) do {
      // Bound the outer solve even when finite Newton corrections fail to reduce the residual.
      // A failure returns NULL instead of silently accepting a frequency outside match_tol.
      if (newton_solve_count >= max_newton_solves) {
        if (verbosity > 0)
          master_printf("MPB failed to match frequency after %d Newton solves\n",
                        max_newton_solves);
        eigvals[band_num - 1] = -1;
        break;
      }

      // A one-cell MPB problem is the plane-wave limit used by mode-decomposition examples.  An
      // exactly (or nearly) converged warm vector plus a tiny k update can make MPB 1.12's default
      // eigensolver stall internally.  Recreate the same deterministic starting vector for
      // subsequent FP32 single-band Newton solves in this degenerate-size case.  FP64, multi-band,
      // and larger mode problems retain their warm start for band continuity and performance.
#if MEEP_SINGLE
      if (newton_solve_count > 0 && eigenmode_grid_points == 1 && band_num == 1) {
        set_random_seed(314159);
        for (int i = 0; i < H.n * H.p; ++i)
          ASSIGN_SCALAR(H.data[i], uniform_random(-1, 1), uniform_random(-1, 1));
        restore_random_seed();
      ...
      ++newton_solve_count;
      double solved_frequency = sqrt(eigvals[band_num - 1]);
      if (!std::isfinite(solved_frequency) || solved_frequency <= 0.0) {
        eigvals[band_num - 1] = -1;
        break;
      }
      if (verbosity > 0)
        master_printf("MPB solved for frequency_%d(%g,%g,%g) "
                      ...
                      G[0][2] * k[0] + G[1][2] * k[1] + G[2][2] * k[2], solved_frequency,
                      num_iters);
      ...
      vgrp /= solved_frequency;

      // return to original size
      evectmatrix_resize(&W[0], band_num, 0);
      evectmatrix_resize(&W[1], band_num, 0);

      if (!std::isfinite(static_cast<double>(vgrp)) || vgrp == 0.0) {
        eigvals[band_num - 1] = -1;
        break;
      }

      frequency_converged =
          !match_frequency || fabs(solved_frequency - frequency) / frequency <= match_tol;
      if (match_frequency && !frequency_converged) {
        // update k via Newton step
        double dkmatch = (solved_frequency - frequency) / vgrp;
        double candidate_kmatch = kmatch - dkmatch;
        if (!std::isfinite(dkmatch) || !std::isfinite(candidate_kmatch)) {
          eigvals[band_num - 1] = -1;
          break;
        }
        mpb_real candidate_k[3] = {k[0], k[1], k[2]};
        if (d == NO_DIRECTION) {
          for (int i = 0; i < 3; ++i) {
            // kdir*candidate_kmatch in reciprocal basis, retaining fixed beta in 2d
            candidate_k[i] = dot_product(R[i], kdir) * candidate_kmatch;
            if (gv.dim == D2) candidate_k[i] += R[i][2] * beta;
          }
        }
        else { candidate_k[d - X] = candidate_kmatch * R[d - X][d - X]; }
        for (int i = 0; i < 3; ++i)
          if (!std::isfinite(static_cast<double>(candidate_k[i]))) {
            eigvals[band_num - 1] = -1;
            break;
          }
        if (eigvals[band_num - 1] < 0) break;
        if (verbosity > 1)
          master_printf("Newton step: group velocity v=%g, kmatch=%g\n", vgrp,
                        candidate_kmatch);
        count_dkmatch_increase += fabs(dkmatch) > fabs(dkmatch_prev);
        dkmatch_prev = dkmatch;
        if (count_dkmatch_increase > 4) {
          eigvals[band_num - 1] = -1;
          break;
        }
        kmatch = candidate_kmatch;
        for (int i = 0; i < 3; ++i)
          k[i] = candidate_k[i];
        update_maxwell_data_k(mdata, k, G[0], G[1], G[2]);
      }
    } while (match_frequency && !frequency_converged);

  if (dp) {
    // compute sum of (kparallel+G)^2 in all the periodic directions
    double k2sum = 0, ktmp = 0;
    int m = 0;
    bool included_z = false;
    LOOP_OVER_DIRECTIONS(v.dim, dd) {
      m = dp->get_g()[dd - X];
      if (dd != dp_normal && eig_vol.in_direction(dd) != 0) {
        ktmp = kpoint.in_direction(dd) + m / eig_vol.in_direction(dd);
        k2sum += ktmp * ktmp;
        included_z = included_z || dd == Z;
      }
    }
    if (((v.dim == D3) && (float(v.in_direction(Z)) == float(1 / a)) &&
         ...
         (real(k[Z]) != 0)) ||
        ((v.dim == D2) && (real(k[Z]) != 0))) {
      if (dp_normal != Z && !included_z) {
        double physical_kz = G[0][2] * k[0] + G[1][2] * k[1] + G[2][2] * k[2];
        k2sum += physical_kz * physical_kz;
      }
    }

    // compute (non-evanescent) kperp from sum of (kparallel+G)^2 OR
    // frequency from sum of kperp^2 and (kparallel+G)^2
    bool dp_mode_valid = true;
    {
      double current_kperp = G[0][dp_normal - X] * k[0] + G[1][dp_normal - X] * k[1] +
                             G[2][dp_normal - X] * k[2];
      if (!std::isfinite(current_kperp)) dp_mode_valid = false;
      // A fixed-k grazing wave carries no power through the requested plane and cannot be scaled
      // to unit flux.  Reject it before maxwell_set_planewave, whose polarization-axis checks can
      // otherwise terminate the process before the zero group velocity is handled below.
      if (!match_frequency && current_kperp == 0.0) dp_mode_valid = false;
      if (match_frequency && dp_mode_valid) {
        vec cen = eig_vol.center();
        double eps_mu = real(get_eps(cen, frequency)) * real(get_mu(cen, frequency));
        double medium_k2 = frequency * frequency * eps_mu;
        double k2 = medium_k2 - k2sum;
        double cutoff_tol = 64 * std::numeric_limits<double>::epsilon() *
                            fmax(fabs(medium_k2), fabs(k2sum));
        if (!std::isfinite(eps_mu) || eps_mu <= 0.0 || !std::isfinite(k2) ||
            k2 <= cutoff_tol) {
          if (am_master())
            master_printf("WARNING: diffraction order for g=(%d,%d,%d) is evanescent, "
                          "at cutoff, or has an invalid medium!\n",
                          dp->get_g()[0], dp->get_g()[1], dp->get_g()[2]);
          dp_mode_valid = false;
        }
        else {
          double delta_kperp = sqrt(k2) - current_kperp;
          mpb_real candidate_k[3] = {k[0], k[1], k[2]};
          if (!std::isfinite(delta_kperp)) dp_mode_valid = false;
          for (int i = 0; i < 3 && dp_mode_valid; ++i) {
            double candidate = static_cast<double>(k[i]) +
                               static_cast<double>(R[i][dp_normal - X]) * delta_kperp;
            candidate_k[i] = static_cast<mpb_real>(candidate);
            if (!std::isfinite(candidate) ||
                !std::isfinite(static_cast<double>(candidate_k[i])))
              dp_mode_valid = false;
          }
          if (dp_mode_valid)
            for (int i = 0; i < 3; ++i)
      ...
      else if (!match_frequency && dp_mode_valid) {
        vec cen = eig_vol.center();
        double eps_mu = real(get_eps(cen, frequency)) * real(get_mu(cen, frequency));
        double k_squared = current_kperp * current_kperp + k2sum;
        if (!std::isfinite(eps_mu) || eps_mu <= 0.0 || !std::isfinite(k_squared) ||
            k_squared <= 0.0)
          dp_mode_valid = false;
        else {
          double frequency_squared = k_squared / eps_mu;
          if (!std::isfinite(frequency_squared) || frequency_squared <= 0.0)
            dp_mode_valid = false;
          else
      ...
      // MPB aborts the entire process if the polarization axis is parallel to the selected
      // k+G.  Validate the final (possibly frequency-matched) physical wavevector here so bad
      // user input follows the ordinary no-mode cleanup/return path instead.
      if (dp_mode_valid) {
        double k_plus_g[3] = {0.0, 0.0, 0.0};
        for (int j = 0; j < 3; ++j)
          for (int i = 0; i < 3; ++i)
            k_plus_g[j] += static_cast<double>(G[i][j]) *
                           (static_cast<double>(k[i]) + dp->get_g()[i]);
        const double *axis = dp->get_axis();
        double axis_norm2 = 0.0, k_plus_g_norm2 = 0.0;
        for (int i = 0; i < 3; ++i) {
          axis_norm2 += axis[i] * axis[i];
          k_plus_g_norm2 += k_plus_g[i] * k_plus_g[i];
        }
        const double cross[3] = {
            axis[1] * k_plus_g[2] - axis[2] * k_plus_g[1],
            axis[2] * k_plus_g[0] - axis[0] * k_plus_g[2],
            axis[0] * k_plus_g[1] - axis[1] * k_plus_g[0],
        };
        const double cross_norm2 =
            cross[0] * cross[0] + cross[1] * cross[1] + cross[2] * cross[2];
        const double parallel_tol2 =
            64.0 * std::numeric_limits<mpb_real>::epsilon() * axis_norm2 * k_plus_g_norm2;
        if (!std::isfinite(axis_norm2) || !std::isfinite(k_plus_g_norm2) ||
            !std::isfinite(cross_norm2) || !std::isfinite(parallel_tol2) ||
            axis_norm2 <= 0.0 || k_plus_g_norm2 <= 0.0 || cross_norm2 <= parallel_tol2) {
          if (am_master())
            master_printf("WARNING: DiffractedPlanewave axis is parallel or ill-conditioned "
                          "relative to k+G\n");
          dp_mode_valid = false;
        }
      }
    ...
    if (am_master() && dp_mode_valid) {
      ...
      for (int i = 0; i < H.n; ++i)
        W[0].data[i] = H.data[H.p - 1 + i * H.p];
      mpb_real dp_kdir[3] = {0, 0, 0};
      dp_kdir[dp_normal - X] = 1;
      maxwell_ucross_op(W[0], W[1], mdata, dp_kdir); // W[1] = (dTheta/dk_perp) W[0]
      mpb_real vscratch;
      evectmatrix_XtY_diag_real(W[0], W[1], &vgrp, &vscratch);
  ...
  if (!std::isfinite(eigval) || eigval <= 0.0 ||
      !std::isfinite(static_cast<double>(vgrp)) || vgrp == 0.0) { // no normalizable mode found
...
void fields::get_eigenmode_coefficients(dft_flux flux, const volume &eig_vol, int *bands,
  ...
  // Pass the caller's cache slot directly to get_eigenmode.  Invalidation must clear the
  // authoritative pointer before destroying its maxwell_data; copying the pointer into a local
  // slot can otherwise leave the monitor cache dangling when an exception unwinds this function.
  void *local_mdata = NULL;
  void **mdata_slot = eigenmode_cache ? eigenmode_cache : &local_mdata;
  struct local_mdata_lifetime {
    void *&data;
    bool owns;
    ~local_mdata_lifetime() {
      if (owns && data) destroy_maxwell_data((maxwell_data *)data);
    }
  } local_mdata_guard = {local_mdata, eigenmode_cache == NULL};
  ...
  for (int nb = 0; nb < num_bands; nb++) {
    for (int nf = 0; nf < num_freqs; nf++) {
      ...
      void *mode_data = get_eigenmode(flux.freq[nf], d, flux.where, eig_vol, band_num, kpoint,
                                      ...
                                      kdom, mdata_slot, dp, cache_dispersive, cache_frequency);
      finished_working();
      if (!mode_data) { // mode not found, assume evanescent
        ...
        if (cscale) cscale[nb * num_freqs + nf] = 0;
        continue;
      }
      struct eigenmode_data_lifetime {
        void *data;
        ~eigenmode_data_lifetime() {
          if (data) destroy_eigenmode_data(data, false);
        }
      } mode_data_guard = {mode_data};
      ...
      get_mode_flux_and_mode_mode_overlaps(
          mode_data, flux, nf, mode_flux, mode_mode);
