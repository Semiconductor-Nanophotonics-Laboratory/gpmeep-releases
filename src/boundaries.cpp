...
void fields::disconnect_chunks() {
  chunk_connections_valid = false;
  destroy_gpu_boundary_topologies();
...
void fields::connect_the_chunks() {
  ...
  FOR_FIELD_TYPES(f) {
    ...
    const int maximum_tag = max_communication_tag();
    const auto next_tag = [&tagto, maximum_tag](int peer) {
      if (peer < 0 || static_cast<std::size_t>(peer) >= tagto.size())
        meep::abort("invalid peer rank while planning boundary exchange");
      const int tag = tagto[static_cast<std::size_t>(peer)];
      if (tag > maximum_tag)
        meep::abort("boundary exchange requires an MPI tag above MPI_TAG_UB");
      ++tagto[static_cast<std::size_t>(peer)];
      return tag;
    };
