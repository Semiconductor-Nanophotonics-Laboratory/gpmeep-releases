...
src_vol::src_vol(component cc, src_time *st, std::vector<ptrdiff_t> &&ind,
    ...
    : c([](component c) -> component {
      ...
      needs_boundary_fix(fix_boundaries), src_t(st), index(std::move(ind)),
      amp(std::move(amps)), amp_l1(0.0) {
  assert(index.size() == amp.size());
  amp_fp32.resize(2 * amp.size());
  for (size_t i = 0; i < amp.size(); ++i) {
    amp_l1 += abs(amp[i]);
    amp_fp32[2 * i] = static_cast<float>(amp[i].real());
    amp_fp32[2 * i + 1] = static_cast<float>(amp[i].imag());
  }
}

void src_vol::set_amplitude(size_t j, std::complex<double> value) {
  amp_l1 += abs(value) - abs(amp[j]);
  amp[j] = value;
  amp_fp32[2 * j] = static_cast<float>(value.real());
  amp_fp32[2 * j + 1] = static_cast<float>(value.imag());
}

void src_vol::set_amplitude(size_t j, double value) {
  set_amplitude(j, std::complex<double>(value, 0.0));
}
...
void fields::add_volume_source(component c, const src_time &src, const volume &where_,
  ...
  // src_vol_chunkloop reads geometry/material coefficients but not field
  // arrays.  Keep device-authoritative fields resident when replacing a
  // source (Python Simulation.change_sources may do this every time step).
  loop_in_chunks(src_vol_chunkloop, (void *)&data, where, c, false, false,
                 false);
  require_component(c);
}
