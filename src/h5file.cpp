...
size_t h5file::root_object_count() {
#if HAVE_HDF5
  const hid_t root = H5Gopen(HID(get_id()), "/");
  CHECK(root >= 0, "could not open HDF5 root group");
  hsize_t count = 0;
  const herr_t status = H5Gget_num_objs(root, &count);
  const herr_t close_status = H5Gclose(root);
  CHECK(status >= 0, "could not count HDF5 root objects");
  CHECK(close_status >= 0, "could not close HDF5 root group");
  CHECK(count <= static_cast<hsize_t>(std::numeric_limits<size_t>::max()),
        "HDF5 root object count does not fit in size_t");
...
#ifdef HAVE_HDF5
  hid_t checked_id = -1;
  // ok() is a probe: callers use its boolean result to report or recover
  // from an expected open failure, so do not also dump HDF5's global error
  // stack to stderr.
  SUPPRESS_HDF5_ERRORS(checked_id = HID(get_id()));
  return checked_id >= 0;
#else
  return false;
#endif
...
static void _write_chunk(hid_t data_id, h5file::extending_s *cur, int rank,
  ...
  if (do_write)
    write_status = H5Dwrite(data_id, datatype, mem_space_id, space_id, H5P_DEFAULT, (void *)data);

  const herr_t mem_close_status = H5Sclose(mem_space_id);
  const herr_t file_close_status = H5Sclose(space_id);
  CHECK(write_status >= 0, "error writing HDF5 data chunk");
  CHECK(mem_close_status >= 0, "error closing HDF5 memory dataspace after writing");
  CHECK(file_close_status >= 0, "error closing HDF5 file dataspace after writing");
#else
  meep::abort("not compiled with HDF5, required for HDF5 output");
#endif
}
...
bool h5file::write_serial_checked(const char *dataname, int rank,
                                  const size_t *dims, const void *data,
                                  bool single_precision) {
#ifdef HAVE_HDF5
  // This deliberately bypasses create_data/write_chunk: those long-standing
  // public methods abort on HDF5 errors, which is unsafe when only rank zero
  // performs I/O and peer ranks are waiting for a failure collective.
  if (!dataname || !dims || !data || rank <= 0 || parallel ||
      (!local && !am_master()))
    return false;
  hid_t file_id = -1;
  SUPPRESS_HDF5_ERRORS(file_id = HID(get_id()));
  if (file_id < 0) return false;

  hsize_t *hdf5_dims = nullptr;
  hid_t space_id = -1;
  hid_t data_id = -1;
  herr_t write_status = -1;
  herr_t data_close_status = 0;
  herr_t space_close_status = 0;
  bool dimensions_valid = true;
  try {
    hdf5_dims = new hsize_t[static_cast<size_t>(rank)];
    for (int dim = 0; dim < rank; ++dim) {
      hdf5_dims[dim] = static_cast<hsize_t>(dims[dim]);
      if (dims[dim] == 0 ||
          static_cast<size_t>(hdf5_dims[dim]) != dims[dim])
        dimensions_valid = false;
    }
    SUPPRESS_HDF5_ERRORS(
        if (dimensions_valid)
          space_id = H5Screate_simple(rank, hdf5_dims, nullptr);
        if (space_id >= 0)
          data_id = H5Dcreate(
              file_id, dataname,
    ...
    if (data_id >= 0) (void)H5Dclose(data_id);
    if (space_id >= 0) (void)H5Sclose(space_id);
    delete[] hdf5_dims;
    throw;
  }
  SUPPRESS_HDF5_ERRORS(
      if (data_id >= 0) data_close_status = H5Dclose(data_id);
      if (space_id >= 0) space_close_status = H5Sclose(space_id););
  delete[] hdf5_dims;
  return dimensions_valid && space_id >= 0 && data_id >= 0 &&
         write_status >= 0 && data_close_status >= 0 &&
         space_close_status >= 0;
#else
  (void)dataname;
  (void)rank;
  (void)dims;
...
bool h5file::flush_and_close_checked() {
#ifdef HAVE_HDF5
  if (parallel || (!local && !am_master()) || HID(id) < 0) return false;
  herr_t flush_status = -1;
  herr_t close_status = -1;
  SUPPRESS_HDF5_ERRORS(
      flush_status = H5Fflush(HID(id), H5F_SCOPE_GLOBAL);
      close_status = H5Fclose(HID(id)););
  if (close_status >= 0) {
    HID(id) = -1;
    if (mode == WRITE) mode = READWRITE;
  }
  return flush_status >= 0 && close_status >= 0;
#else
  return false;
#endif
}
...
static void _read_chunk(hid_t data_id, int rank, const size_t *chunk_start,
  ...
  herr_t read_status = 0;
  if (do_read)
    read_status = H5Dread(data_id, datatype, mem_space_id, space_id, H5P_DEFAULT, (void *)data);

  const herr_t mem_close_status = H5Sclose(mem_space_id);
  const herr_t file_close_status = H5Sclose(space_id);
  CHECK(read_status >= 0, "error reading HDF5 data chunk");
  CHECK(mem_close_status >= 0, "error closing HDF5 memory dataspace after reading");
  CHECK(file_close_status >= 0, "error closing HDF5 file dataspace after reading");
#else
