...
public:
  ...
  // Internal non-virtual CUDA hook. Keeping this out of the virtual table
  // preserves susceptibility ABI while allowing update_pols to dispatch the
  // standard Lorentzian/Drude implementation.
  bool update_P_cuda(gpu::detail::resident_cache *cache,
...
public:
  ...
  void write(const char *dataname, int rank, const size_t *dims, void *data,
             bool single_precision = true);
  // Abort-free serial dataset publication used by collective callers that
  // must convert a master-only HDF5 failure into a symmetric MPI exception.
  // Every HDF5 handle opened here is closed before return. The final method
  // flushes and closes the file and reports either failure without aborting.
  bool write_serial_checked(const char *dataname, int rank,
                            const size_t *dims, const void *data,
                            bool single_precision = true);
  bool flush_and_close_checked();
  ...
  // Number of objects linked directly below the HDF5 root group.  This is
  // useful for validating an exact output inventory in regression tools.
  size_t root_object_count();
...
// ABI-safe queries for the library-owned manager implementation. These avoid
// extending the public comms_manager vtable in a drop-in libmeep build.
bool comms_supports_cuda_device_buffers(const comms_manager *manager);
// Query the collectively validated completion strategy without extending the
// public comms_manager vtable.
...
// CUDA-aware managers may post receives before resident GPU preparation and
// post sends only after the producer event is complete. The split API keeps
// callback/request planning transactional while allowing MPI/GPU overlap.
// Non-CUDA-aware managers reject these entry points.
void comms_start_cuda_device_receives(comms_manager *manager);
void comms_start_cuda_device_sends(comms_manager *manager);
struct comms_overlap_statistics {
  std::uint64_t eager_receive_start_calls;
  std::uint64_t eager_send_start_calls;
  std::uint64_t eager_receive_requests;
  std::uint64_t eager_send_requests;
};
void reset_comms_overlap_statistics() noexcept;
comms_overlap_statistics get_comms_overlap_statistics() noexcept;
// Complete all posted transfers while retaining manager-owned allocation
// capacity for a topology-stable replay. This free function keeps the public
// comms_manager vtable unchanged for drop-in ABI compatibility.
...
public:
  ...
  /* Return point-major Npoint x Nfreq x 6 far fields.  Unlike repeated
     farfield calls, this performs one CUDA transform and one MPI reduction. */
  std::complex<double> *farfields(const vec *points, size_t point_count,
...
// Internal partition used by the CUDA communication/compute-overlap path.
// It is declared here because fields and fields_chunk coordinate the two
// launches, but it is not exposed through the language bindings.
enum class cuda_curl_region : unsigned char { full, interior, shell };
...
public:
  ...
  // Monitors removed from timestep updates by fields::clear_dft_monitors()
  // but still owned for lifecycle purposes until this chunk is destroyed.
  dft_chunk *detached_dft_chunks;
  ...
  friend class fields;
  friend class fields_chunk_test_access;
  ...
  bool cuda_whole_update_eh_overlap_eligible(field_type ft,
...
private:
  ...
  bool cuda_step_db_eligible(field_type ft, std::string *reason) const;
  bool cuda_halo_curl_overlap_eligible(field_type ft,
                                       size_t *full_points,
                                       size_t *interior_points,
                                       bool *rejected_small,
                                       std::string *reason) const;
  bool step_db(field_type ft);
  bool step_db_cuda_region(field_type ft, cuda_curl_region region);
...
public:
  ...
  // Internal variant for callbacks such as source construction that operate
  // only on geometry/material storage and never dereference host field arrays.
  void loop_in_chunks(field_chunkloop chunkloop, void *chunkloop_data, const volume &where,
                      component cgrid, bool use_symmetry, bool snap_unit_dims,
                      bool sync_resident_fields);
  ...
  void get_mode_mode_overlap(void *mode1_data, void *mode2_data, dft_flux flux,
                             std::complex<double> overlaps[2]);
  void get_mode_flux_and_mode_mode_overlaps(
...
private:
  ...
  // step.cpp
  void cuda_backend_preflight() const;
  void cuda_step_preflight() const;
  bool cuda_dft_preflight(std::string *reason) const;
  enum class boundary_eh_overlap { none, H, E, D_curl };
  bool step_boundaries_impl(field_type ft,
                            boundary_eh_overlap overlap);
  bool cuda_whole_update_eh_overlap_eligible(field_type ft,
                                              std::string *reason) const;
  void phase_material();
  void step_db(field_type ft);
  void step_db_cuda_region(field_type ft, cuda_curl_region region);
  bool cuda_halo_curl_overlap_eligible(field_type ft,
                                       size_t *full_points,
                                       size_t *interior_points,
                                       bool *rejected_small,
...
public:
  ...
  // Owner-specific automatic-backend result.  The process-wide gpu API
  // reports the most recently activated owner; these accessors remain stable
  // when multiple fields objects interleave CPU and CUDA steps.
  bool gpu_cuda_execution_selected() const;
  const char *gpu_execution_diagnostic() const;

private:
  struct gpu_boundary_topology;
  struct gpu_boundary_topology_registry;
  static gpu_boundary_topology_registry &
  gpu_boundary_topology_registry_instance();
  gpu_boundary_topology *get_gpu_boundary_topology(field_type ft) const;
  void replace_gpu_boundary_topology(field_type ft,
                                     gpu_boundary_topology *topology);
  bool gpu_backend_preflight_is_cached() const;
  void cache_gpu_backend_preflight(bool cuda_execution,
                                   const std::string &diagnostic,
                                   bool feature_fallback = false) const;
  void prepare_gpu_owner_for_cpu() const;
  bool gpu_step_preflight_is_cached() const;
  void cache_gpu_step_preflight() const;
  void validate_gpu_mpi_transport() const;
  void validate_gpu_device_assignment() const;
  timing_scope with_timing_scope(time_sink sink);
  void destroy_gpu_boundary_topologies(bool erase_owner = false);
...
// Check, in linear time, that gvs is exactly the deterministic partition
// produced by split_into_tiles for loop_tile_base.
void check_tiles(grid_volume gv, const std::vector<grid_volume> &gvs,
                 const size_t loop_tile_base);
