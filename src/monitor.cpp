...
complex<double> fields_chunk::get_field(component c, const ivec &iloc) const {
  if (!is_mine() || !f[c][0]) return 0.0;
  const size_t index = gv.index(c, iloc);
#if MEEP_SINGLE
  float resident_real = 0.0f;
  float resident_imaginary = 0.0f;
  const unsigned int resident_components =
      gpu::detail::read_resident_field_point_fp32_for_owner(
          this, f[c][0], f[c][1], index, &resident_real,
          &resident_imaginary);
  if (resident_components)
    return complex<double>(
        resident_components & 1u ? resident_real : f[c][0][index],
        resident_components & 2u
            ? resident_imaginary
            : (f[c][1] ? f[c][1][index] : 0.0));
#endif
  return f[c][1] ? getcm(f[c], index) : f[c][0][index];
}
