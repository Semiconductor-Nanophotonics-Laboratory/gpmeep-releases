...
void fields::loop_in_chunks(field_chunkloop chunkloop, void *chunkloop_data, const volume &where,
  ...
  loop_in_chunks(chunkloop, chunkloop_data, where, cgrid, use_symmetry,
                 snap_empty_dimensions, true);
}

void fields::loop_in_chunks(field_chunkloop chunkloop, void *chunkloop_data, const volume &where,
                            component cgrid, bool use_symmetry, bool snap_empty_dimensions,
                            bool sync_resident_fields) {
  // Generic chunk callbacks include integrations, array slices, and HDF5
  // output, all of which may read host field pointers directly.  A small set
  // of internal geometry-only callbacks explicitly opt out of this publish.
  if (sync_resident_fields)
