...
void fields::fix_boundary_sources() {
  ...
  // Boundary repair mutates only source indices/amplitudes.  Invalidating the
  // complete owner cache here used to publish every device-authoritative
  // field and rebuild all persistent CUDA plans on each Python
  // change_sources() call. Only source/LDOS descriptor snapshots that can
  // retain the repaired profile pointers are invalidated now.
  for (int i = 0; i < num_chunks; ++i)
    FOR_FIELD_TYPES(ft) for (const src_vol &source : chunks[i]->sources[ft])
      gpu::detail::discard_resident_source_profile_for_owner(
          chunks[i], source.indices_data(),
          source.amplitudes_fp32_data());
