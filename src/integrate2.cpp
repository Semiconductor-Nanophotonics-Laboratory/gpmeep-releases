...
class distributed_integrate2_failure_guard {
public:
  explicit distributed_integrate2_failure_guard(bool armed) : armed_(armed) {}

  ~distributed_integrate2_failure_guard() {
    if (armed_ && std::uncaught_exception())
      meep::abort(
          "rank-local failure during distributed integrate2; aborting the "
          "communicator to prevent an MPI collective deadlock");
  }
...
complex<double> fields::integrate2(const fields &fields2, int num_fvals1,
  ...
  // integrate2 performs two distributed reductions after rank-local CUDA
  // publication, allocation, and user-callback work.  If any of those local
  // operations throws on only one rank, fail the communicator instead of
  // leaving its peers blocked in max_to_all/sum_to_all below.
  const distributed_integrate2_failure_guard distributed_failure_guard(
      count_processors() > 1);

  if (!equal_layout(fields2))
    meep::abort("invalid call to integrate2: fields must have equal grid layout");
  for (int i = 0; i < num_chunks; ++i)
    if (chunks[i]->n_proc() != fields2.chunks[i]->n_proc())
      meep::abort(
          "invalid call to integrate2: fields must have identical per-chunk "
          "process ownership (chunk %d: %d != %d)",
          i, chunks[i]->n_proc(), fields2.chunks[i]->n_proc());
  ...
  // loop_in_chunks() publishes this fields object's device-authoritative
  // mirrors before invoking a generic host callback.  integrate2 reads a
  // second fields object directly from the same callback, so publish that
  // object's rank-local chunks as well.  Without this step a retained first
  // simulation can still contain its only up-to-date values on the GPU after
  // Simulation.reset_meep(), causing the two-field integral to observe the
  // old zero-filled host allocation.
  for (int i = 0; i < fields2.num_chunks; ++i)
    if (fields2.chunks[i]->is_mine())
      gpu::detail::sync_resident_cache_for_owner(fields2.chunks[i]);
