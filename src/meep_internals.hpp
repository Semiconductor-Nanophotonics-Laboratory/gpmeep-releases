...
public:
  ...
  const ptrdiff_t *indices_data() const { return index.data(); }
  std::complex<double> amplitude(size_t j) const { return amp[j]; };
  void set_amplitude(size_t j, std::complex<double> a);
  void set_amplitude(size_t j, double a);
  const std::complex<double> &amplitude_at(size_t pos) const { return amp[pos]; }
  double amplitude_l1() const { return amp_l1; }
  // Interleaved real/imaginary FP32 amplitudes used as a stable resident
  // source profile. This is updated whenever the canonical FP64 amplitude is
  // mutated.
  const float *amplitudes_fp32_data() const { return amp_fp32.data(); }
...
private:
  ...
  std::vector<float> amp_fp32;             // interleaved real/imaginary amplitudes
  double amp_l1;                           // cached sum of absolute amplitudes
};
