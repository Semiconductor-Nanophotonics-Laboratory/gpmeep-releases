...
void fields::load_fields_chunk_field(h5file *h5f, bool single_parallel_file,
                                     const std::string &field_name,
                                     FieldPtrGetter field_ptr_getter) {
  // A checkpoint load may delete or replace field arrays while their chunk
  // remains alive. Preserve any device-authoritative arrays first, then drop
  // address-keyed mirrors before the host allocations can change.
  for (int i = 0; i < num_chunks; ++i) {
    gpu::detail::sync_resident_cache_for_owner(chunks[i]);
