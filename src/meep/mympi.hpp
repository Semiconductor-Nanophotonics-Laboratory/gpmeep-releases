...
int my_rank();
// Rank topology within a physical shared-memory node. CUDA uses this to map
// one MPI rank to one visible GPU by default, independently of communicator
// splitting performed by divide_parallel_processes().
int my_node_rank();
int count_node_processors();
// Initialize/finalize MPI resources used by distributed CUDA.  C++ programs
// call these through meep::initialize; MPI-enabled Python calls them after
// mpi4py has initialized MPI.  Both operations are collective over
// MPI_COMM_WORLD and idempotent.
void initialize_distributed_gpu_runtime();
void finalize_distributed_gpu_runtime();
// Collective over the active Meep communicator. CUDA/MIG UUIDs are globally
// unique, so empty or duplicate identifiers anywhere in the communicator are
// invalid unless both duplicate ranks explicitly allow sharing.
bool distributed_device_identifiers_are_unique(const char *identifier,
                                               bool allow_duplicates);
// Noncollective world-wide claim used to prevent separate Meep
// subcommunicators from silently sharing one CUDA/MIG UUID.
bool claim_distributed_device_identifier(const char *identifier,
                                         bool allow_duplicates);
void release_distributed_device_identifier();
// Collective preflight for a distributed CUDA fields object. Freezes one
// wire-compatible pinned or CUDA-aware transport and one completion policy
// across all ranks.
void validate_distributed_mpi_transport();
// Largest point-to-point tag supported by the active Meep communicator.
